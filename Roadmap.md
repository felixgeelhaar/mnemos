...
- [x] **axi-go evidence** — three slices shipped: (a) JSONL evidence sink via PR #67 (every kernel event lands in `MNEMOS_AXI_EVIDENCE_LOG` for cross-session audit, fan-out via `multiAxiPublisher` alongside the existing bolt log); (b) LLM token reporting plumbed from `internal/llm.Response` → `extract.LLMEngine.OnUsage` → `pipeline.Extractor.LastUsage()` → `mcpProcessTextOutput.{Input,Output}Tokens` → a `Kind: "llm"` capability evidence record so `MaxTokens` engages; new `MNEMOS_AXI_MAX_TOKENS` env knob; (c) approval-flow exercise pinning Approve + Reject for any future `EffectWriteExternal` tool. SQLite-backed persistence intentionally skipped — JSONL+`jq` covers the audit need without a second backend.
