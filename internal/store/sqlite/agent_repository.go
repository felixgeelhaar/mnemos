...
func (r AgentRepository) Create(ctx context.Context, a domain.Agent) error {
	...
	_, err = r.db.ExecContext(ctx,
		`INSERT INTO agents (id, name, owner_id, scopes_json, status, created_at) VALUES (?, ?, ?, ?, ?, ?)`,
		a.ID, a.Name, a.OwnerID, scopesJSON, string(a.Status), a.CreatedAt.UTC().Format(time.RFC3339Nano),
	)
	if err != nil {
...
func (r AgentRepository) GetByID(ctx context.Context, id string) (domain.Agent, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, name, owner_id, scopes_json, status, created_at FROM agents WHERE id = ?`, id)
...
func (r AgentRepository) List(ctx context.Context) ([]domain.Agent, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, name, owner_id, scopes_json, status, created_at FROM agents ORDER BY created_at ASC`)
	if err != nil {
...
func scanAgent(s agentRowScanner) (domain.Agent, error) {
	...
	if err := s.Scan(&a.ID, &a.Name, &a.OwnerID, &scopesRaw, &statusStr, &createdAt); err != nil {
